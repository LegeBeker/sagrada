...
public class DieDropTarget extends StackPane {
    ...
    public DieDropTarget(final ViewController view) {
        ...
        this.setOnDragDropped(event -> {
            ...
            if (amountPlacedDie > 0 && !view.getSelectedToolcardName().equals("runningPliers")) {
                this.view.displayError(
                        "Je hebt al een dobbelsteen geplaatst deze ronde, eindig de ronde om nog eens te plaatsen.");
                return;
            } else if (view.getSelectedToolcardName().equals("runningPliers")) {
                if (amountPlacedDie > 1) {
                    this.view.displayError("Je hebt al 2 stenen geplaatst d.m.v. de gereedschapskaart.");
                } else {
